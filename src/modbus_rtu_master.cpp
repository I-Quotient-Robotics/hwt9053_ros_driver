...
ModbusRTUMaster::ModbusRTUMaster(const std::string portNmae, const uint32_t baudRate) {
  try {
    ...
    ROS_ERROR_STREAM("Unable to open serial port:" << portNmae);
    return;
  }

  ROS_INFO_STREAM("open serial port:" << portNmae << " successful!!");
...
uint8_t ModbusRTUMaster::SetMultipleRegisters(const uint8_t slave_id, const uint16_t address, const uint16_t length, uint16_t *data, uint16_t delay_ms) {
  if (!com_.isOpen()) {
  ...
  if (receive_length != read_length) {
    ROS_ERROR("Failed to read message!!");
    com_.flush();
    return 0;
  }
  if (receive_buffer[0] != slave_id) {
    ROS_ERROR("Message ID error!!");
    com_.flush();
    return 0;
  }
  if (receive_buffer[1] != function_num) {
    ROS_ERROR("Message fuction number error!!");
    com_.flush();
    return 0;
  }
  ...
  if (temp_address != address) {
    ROS_ERROR("Message start address error!!");
    com_.flush();
    return 0;
  }

  if (temp_length != length) {
    ROS_ERROR("Message read length error!!");
    com_.flush();
    return 0;
  }
  ...
  if (rec_crc != rec_crc_) {
    ROS_ERROR("Message crc check error!!");
    com_.flush();
    return 0;
  }
...
uint8_t ModbusRTUMaster::GetMultipleRegisters(const uint8_t slave_id, const uint16_t address, const uint16_t length, uint16_t *data, uint16_t delay_ms) {
  if (!com_.isOpen()) {
    ROS_ERROR("serial not open!!");
    return 0;
  }
  ...
  if (receive_buffer[2] != (length * 2)) {
    ROS_ERROR("Message data length error!!");
    com_.flush();
    return 0;
  }
